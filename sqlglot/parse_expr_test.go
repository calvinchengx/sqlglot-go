...
func TestColumnPositionAndIdentity(t *testing.T) {
	...
	// The short form has nowhere to put CYCLE, ON NULL or ALWAYS.
	...
	// DuckDB has no auto-numbering column of its own, and the reference does
	// not refuse over it -- it drops the constraint silently and writes the
	// rest of the column as though it had never been there.
	duckdbNoIdentity := "CREATE TABLE t (a INT IDENTITY)"
	e, err := ParseOne(duckdbNoIdentity, "tsql")
	if err != nil {
		t.Fatalf("ParseOne(%q): %v", duckdbNoIdentity, err)
	}
	if got, err := Generate(e, "duckdb"); err != nil || got != "CREATE TABLE t (a INT)" {
		t.Errorf("%q wrote %q (%v), want %q", duckdbNoIdentity, got, err, "CREATE TABLE t (a INT)")
	}
}
...
// Materialize's own MAP grammar and its SERIAL-column expansion, in the
...
func TestMaterializeMapAndSerialShapes(t *testing.T) {
	for _, c := range []struct{ sql, dialect, want string }{
		...
		{"SELECT LIST(1, 2, 3)", "postgres", ""},
		{"CREATE TABLE t (a SMALLSERIAL)", "materialize", "CREATE TABLE t (a SMALLINT NOT NULL)"},
		{"CREATE TABLE t (a BIGSERIAL)", "materialize", "CREATE TABLE t (a BIGINT NOT NULL)"},
	} {
		want := c.want
		if want == "" {
			want = c.sql
		...
		if got, err := Generate(tree, c.dialect); err != nil || got != want {
			t.Errorf("%s wrote %q (%v), want %q", c.sql, got, err, want)
		}
	}
	for _, sql := range []string{
		"SELECT MAP['a' => 1, 'b' => MAP['c' => 2",
		"SELECT MAP(1, 2)",
		"SELECT MAP['a' 1]",
	} {
		if _, err := ParseOne(sql, "materialize"); err == nil {
			t.Errorf("%s was read; it should be refused", sql)
		}
	}
}
